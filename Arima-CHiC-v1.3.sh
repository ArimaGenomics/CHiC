...
#       [-n minNPerBait] [-b maxLBrownEst] [-t threads] [-v] [-h] \n"
usage_Help="Usage: ${0##*/} [-W run_hicup] [-Y run_bam2chicago] [-Z run_chicago] [-P run_plot]
              [-A bowtie2] [-X bowtie2_index_basename] [-d digest] [-H hicup_dir] [-C chicago_dir]
              [-I FASTQ_string] [-o out_dir] [-p output_prefix] [-R RMAP] [-B BAITMAP] [-D design_dir]
...
#maxLBrownEst_Help="* [-b maxLBrownEst]: the distance range to be used for estimating the Brownian
...
# while getopts "A:B:b:C:d:D:hH:I:m:M:n:O:o:P:p:R:r:s:t:vW:X:Y:Z:" opt; do
while getopts "A:B:C:d:D:hH:I:O:o:P:p:R:r:t:vW:X:Y:Z:" opt; do
    case $opt in
...
if ! [[ "$maxLBrownEst" =~ ^[0-9]+$ ]]; then
    echo "The maxLBrownEst must be an integer (-b)!"
    printHelpAndExit 1
fi
...
if [[ ! -f "$washU_text" || ! -f "$washU_track" ]]; then
    echo "ERROR: No *_washU_text.txt or *_washU_track.txt file can be found in $plot_dir"
    echo "CHiCAGO was not finished successfully. Please check the log file."
    exit 1
fi
...
loop_file=$plot_dir"/"${output_prefix}".cis_le_2Mb.bedpe"
grep -v fdr $plot_dir"/"${output_prefix}".cis_le_2Mb.longrange" > $loop_file
...
if [ $organism == "hg19" ]; then
  BED="/oasis/tscc/scratch/xiangz/CHiCAGO/design/GW_PC_S3207364_S3207414_hg19.uniq.bed"
  #genome_size=3095693983
elif [ $organism == "hg38" ]; then
  BED="/oasis/tscc/scratch/xiangz/CHiCAGO/design/GW_PC_S3207364_S3207414.uniq.bed"
  #genome_size=3088286401
elif [ $organism == "mm9" ]; then
  BED="/oasis/tscc/scratch/xiangz/CHiCAGO/design/mm9/mouse_GW_PC_S3207063_S3207103_mm9.uniq.bed"
  #genome_size=2654911517
elif [ $organism == "mm10" ]; then
  BED="/oasis/tscc/scratch/xiangz/CHiCAGO/design/mm10/mouse_GW_PC_S3207063_S3207103.uniq.bed"
...
timestamp=`date '+%Y/%m/%d %H:%M:%S'`
echo -e "\nHiCUP, CHiCAGO and Arima QC pipeline finished successfully! [$timestamp]"
